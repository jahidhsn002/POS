...
class Receivings extends Secure_area
{
	...
	function complete()
	{
		...
		if($supplier_id!=-1)
		{
			...
			$data['type'] = 'supplier';
			$data['due_ammount'] = $this->receiving_lib->get_total() - $this->input->post('amount_tendered');
			$data['paid_ammount'] = $this->input->post('amount_tendered');
			if($this->due->check($supplier_id,'supplier') == false){
				$this->due->insert($data);
			}else{
				$ammount = $this->due->check($supplier_id,'supplier');
				$data['due_ammount'] = $ammount[0] + $data['due_ammount'];
				$data['paid_ammount'] = $ammount[1] + $data['paid_ammount'];
				$this->due->update($data);
			}
			//End Mine
